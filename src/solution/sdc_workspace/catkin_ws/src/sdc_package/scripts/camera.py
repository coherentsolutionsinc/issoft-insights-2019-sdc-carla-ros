...
# 1. (Above) Add indicator that node should be run by python
...
class CameraTester(object):
    ...
    def __init__(self):
        ...
        # 2. Init node - give node an unique name - overwritten from launch file
        rospy.init_node('camera_tester')

        # wait master node is initialized and record start time
        self.wait_master_initialization()
        
        # 3. subscribe to camera images and provide callback
        rospy.Subscriber('/carla/ego_vehicle/camera/rgb/front/image_color', Image, self.process_image)
    ...
    # 4. Write callback method for the subscriber
    ...
    def process_frame(self):
        ...
        # 5. Add sample image processing - for example filters useful to lanes detection - uncomment line below    
        frame = pipline(frame)      
